...
public class Animal implements ParkGUI {
	...
	/* isClose() method tests to see if animal is in a specific
	 * range to another anamal
	 *  @param otherAnimal is Animal object, method tests
	 *  if otherAnimal distance to this.animal is within range
	 *  @param range, gives certain range that distance should be
	 *  
	 *  @return true if animals are within range, false otherwise
	 * 
	 */
	public boolean isClose(Animal otherAnimal, int range) {
		if (otherAnimal.distance(otherAnimal) <= range) {
			return true;
		} else {
			return false;
		}
