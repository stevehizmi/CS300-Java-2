...
public class Sequence implements Iterable<Integer> {
	 ...
	 * 
	 * @param command
	 *            array of integers that represents the user command line to
	 *            generate a sequence
	 * 
	 */
	public Sequence(int[] command) {
		...
		case ARITHMETIC: // call the constructor of the ArithmeticSequenceGenerator class to create
			// an iterator over an arithmetic sequence with initial element command[1],
			// common
			...
			break;
		case GEOMETRIC:
			// call constructor of GeometricSequenceGenerator class to create
			// an iterator over a geometric sequence with initial element
			// command[1] , ratio command[2], size command[3]
			sequenceIterator = new GeometricSequenceGenerator(command[1], command[2], command[3]);
			break;
		case FIBONACCI:
			// call the constructor of FibonacciSequenceGenerator to create an iterator
			// over a fibonacci sequence with size command[3].
			sequenceIterator = new FibonacciSequenceGenerator(command[1]);
			break;
		case DIGIT_PRODUCT:
			// call the constructor of DigitProductGenerator to create
			// an iterator over a digit product sequence with initial
			// element command[1] and size command[2]

			DigitProductSequenceGenerator test = new DigitProductSequenceGenerator(command[1], command[2]);
			sequenceIterator = test.getIterator();
			break;
		}
	}
	...
	public String toString() {
		...
		// Use a for-each loop to traverse the sequence and add the different numbers of
		// the sequence
	 ...
	 * @return true if the userCommand contains a syntax error with respect to the
	 *         program specification, false otherwise
	 */
	private static boolean checkCommandSyntax(String[] userCommand) {
		// TODO add the time complexity of this method, where the problem size N is the
		// length
