...
/**
 * This class represents a generator for an arithmetic progression This class
...
public class ArithmeticSequenceGenerator implements Iterator<Integer> {
	...
	public ArithmeticSequenceGenerator(int init, int diff, int size) {
		...
		// check for the validity of init (>=0) and diff (>0), throw an
		// IllegalArgumentException
		// if these two parameters are not valid
		if (init < 0 || diff <= 0)
			throw new IllegalArgumentException(
					"WARNING: " + "The starting element and the common difference for an arithmetic progression "
							+ "should be POSITIVE!");
	...
	/**
	 * Returns the next element in this arithmetic sequence iteration with respect
	 * to the numbers generated so far
	 * 
